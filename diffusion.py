...
## Most of the functions of diffusion have been borrowed 
# and modified from https://github.com/lucidrains/med-seg-diff-pytorch
...
class Conditioning(nn.Module):
    ...
    def forward(self, x, c):
        ...
        c = self.block(c)
        c = adaptive_instance_normalization(c, x)
...
class Unet(nn.Module):
    ...
    def forward(
        ...
        x = adaptive_instance_normalization(c, x)
...
class Diffusion(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        ddim_sampling_eta = 1. # 1 means DDPM sampling, 0 means DDIM
    ):
        super().__init__()
        ...
        alphas_cumprod_prev = F.pad(alphas_cumprod[:-1], (1, 0), value = 1.) #torch.from_numpy(np.append(1., alphas_cumprod[:-1].numpy()))
    ...
    def p_losses(self, x_start, t, cond, noise = None):
        ...
        return F.mse_loss(model_out, target) #+ edge_aware_loss(model_out, target) + self.focal_freq_loss(model_out, target)
...
def train_LDM(device = "cuda:0", NUM_EPOCHS = 1000):

    wandb.init(
        project="CISC881-courseproject",
        entity="debadityashome",
        id="latent-ddpm-AdaIN"
    )

    encoder_IHC = Encoder()
    encoder_IHC.load_state_dict(torch.load("perceptualMMD_encoder_IHC_L1.pth"))

    decoder_IHC = Decoder()
    decoder_IHC.load_state_dict(torch.load("perceptualMMD_decoder_IHC_L1.pth"))

    encoder_HER = Encoder()
    encoder_HER.load_state_dict(torch.load("perceptualMMD_encoder_HER_L1.pth"))
    #decoder_HER = Decoder().load_state_dict(torch.load("perceptualMMD_decoder_HER_L1.pth"))

    ## Freeze the IHC and HER autoencoders
    for param in encoder_IHC.parameters():
        param.requires_grad = False
        
    for param in decoder_IHC.parameters():
        param.requires_grad = False
        
    for param in encoder_HER.parameters():
        param.requires_grad = False
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for (input_img, target_img) in tqdm(training_generator):
            
            input_img.to(device)
            target_img.to(device)
            
            with torch.no_grad():
                ihc_z = encoder_IHC(target_img)
                her_z = encoder_HER(input_img)
            loss = diffusion(ihc_z, her_z)
        ...
        if epoch % 1 == 0:
            ...
            for (input_img, target_img) in tqdm(test_generator):
                ...
                with torch.no_grad():
                    her_z = encoder_HER(input_img)
                    pred = diffusion.sample(her_z).cpu().detach()
                    pred = decoder_IHC(pred)
                
                psnr_test_metric.update(pred, target_img)
                ssim_test_metric.update(pred, target_img)
            ...
            wandb.log(
                {'pred-input-target': [wandb.Image(pred[0, :, :, :].permute(1, 2, 0).numpy()), wandb.Image(input_img[0, :, :, :].permute(1, 2, 0).numpy()),
                                wandb.Image(target_img[0, :, :, :].permute(1, 2, 0).numpy())]}
            )
